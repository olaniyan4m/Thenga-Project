...
self.addEventListener('push', (event) => {
    event.waitUntil(
        self.registration.showNotification('Thenga Customer', {
...
self.addEventListener('sync', event => {
    ...
    if (event.tag === 'cart-sync') {
        event.waitUntil(
            (async () => {
                console.log('[Service Worker] Performing background sync for cart-sync');
                return new Promise(resolve => setTimeout(resolve, 2000));
            })()
        );
    }
    if (event.tag === 'favorites-sync') {
        event.waitUntil(
            (async () => {
                console.log('[Service Worker] Performing background sync for favorites-sync');
                return new Promise(resolve => setTimeout(resolve, 2000));
            })()
        );
    }
...
self.addEventListener('periodicsync', event => {
    ...
    if (event.tag === 'products-sync') {
        event.waitUntil(
            (async () => {
                console.log('[Service Worker] Performing periodic background sync for products');
                return new Promise(resolve => setTimeout(resolve, 2000));
            })()
        );
    }
